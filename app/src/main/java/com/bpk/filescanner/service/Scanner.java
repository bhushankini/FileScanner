package com.bpk.filescanner.service;
...
import com.bpk.filescanner.handler.NotificationHandler;
...
public class Scanner extends Service implements Runnable{
    ...
    private void startScan(){
        ...
        if((!isScanning && !isDone) || (!isScanning && isDone)){
            mNotificationHandler.initialize(NOT_ID,"Scanned: "+currentStatus.totalMB + "MBs",this);
           reStart();
        }
    ...
    private void stopScan(){
          ...
          isDone     = true;
        //isPaused   = true;
        mNotificationHandler.updateMessage(NOT_ID, "Scanning Stopped!", this);
    }
    ...
    private void sendUpdate(){
        ...
        mNotificationHandler.updateMessage(NOT_ID,"Scanned: "+currentStatus.totalMB + "MBs",this);
    ...
    public void run() {
        ...
        sendUpdate();
        mNotificationHandler.updateMessage(NOT_ID, "Scanning Done!", this);
    }
    ...
    private void process(String name,String ext,long size){
        ...
        currentStatus.averageFileSize = avgD;

        currentStatus.totalMB = (int)(((double)scannedBytesSoFar/(double)1000000));
    ...
    private void filterFileData(String name,String ext,long size){
        int sizeIndex = fitLong(currentStatus.biggestTenFileSizes, size);
        retract(currentStatus.biggestTenFileNames,name,sizeIndex);
    }
